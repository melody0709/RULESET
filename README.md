...
| `AI/ruleset_self_AI.yaml` | `🅰️AI` | 第 6 条 | 36 | AI 主域（Claude / Cursor / xAI / Gemini 入口 / Perplexity / OpenRouter …）。`colab.*` 与 `huggingface.co` / `hf.co` 与 `AI_bulk` **有意重复**（本表被单独引用时语义才完整），勿按"重复项"删 |
